 ...
 */
  ...
  /*incrementing recent time*/
  rp->p_recent_time += rp->p_quantum_size-rp->p_ticks_left;
 ...
 */
  ...
  for (q=USER_Q; q<=MIN_USER_Q; q++){
    if(rp=rdy_head[q] !=NIL_PROC)
      {
    for(ptr = &rdy_head[q]; *ptr!=NIL_PROC; ptr=&(*ptr)->p_nextready){
      if((*ptr)->p_recent_time < rp->p_recent_time)
	rp=*ptr;
    }
    next_ptr = rp;
    if(priv(rp)->s_flags&BILLABLE)
 ...
 */
  ...
  for (rp=BEG_PROC_ADDR; rp<END_PROC_ADDR; rp++) {
	...
	/*decays the recent time so it doesn't go too high*/
	if (rp->p_recent_time > 10)
	  rp->p_recent_time = rp->p_recent_time / 2;
	unlock(5);
      }
  }
#if DEBUG
